...
class SKNet(nn.Module):
    def __init__(self, layers = [3, 5, 8], layer_strides = [1, 2, 2],num_filters=[64,128,256]):
        ...
        self.stage_1 = self._make_layer(num_filters[0], int(num_filters[0]*2), num_filters[0], nums_block=layers[0], stride=layer_strides[0])
        self.stage_2 = self._make_layer(num_filters[0], int(num_filters[0]),num_filters[1], nums_block=layers[1], stride=layer_strides[1])
        self.stage_3 = self._make_layer(num_filters[1], int(num_filters[1]), num_filters[2], nums_block=layers[2], stride=layer_strides[2])
...
def SKNet50(layers, layer_strides,num_filters):
    return SKNet(layers=layers, layer_strides=layer_strides,num_filters=num_filters)

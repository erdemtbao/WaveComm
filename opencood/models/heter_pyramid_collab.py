...
class HeterPyramidCollab(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        # x_ll_concat = torch.cat([x_ll, x_lh, x_hl, x_hh], dim=1).to(dtype=torch.float32) #[4, 64*3, 64, 128]
        # x_ll_concat = eval(f"self.conv_highfreq")(x_ll_concat) #[4, 64, 64, 128]
        # reconstructed_feature  = self.wave_generator(x_ll_concat)
        ...
        fused_feature, occ_outputs = self.pyramid_backbone.forward_collab(
                                                reconstructed_feature, #  heter_feature_2d
                                                record_len, 
                                                affine_matrix, 
                                                agent_modality_list, 
                                                self.cam_crop_info
    ...
    def get_distillation_loss(self, output_dict):
        ...
        # ssim loss
        ssim_loss = 1 - ssim(reconstructed_feature,original_feature, data_range=1.0)
        ...
        # print(perceptual_loss,ssim_loss,recon_loss)
        # 5. 组合所有损失
        total_loss = {
            'recon_loss': self.distill_cfg['lambda_recon'] * (1.0 * recon_loss + 1.0* ssim_loss + 0.1 * perceptual_loss),
            # 'recon_loss': self.distill_cfg['lambda_recon'] * (recon_loss + 1.0 * ssim_loss + 0.1 * perceptual_loss),
    ...
    #     # 1. 改进的重建损失
    #     recon_loss = (
    #         0.84 * F.l1_loss(reconstructed_feature, original_feature) +
    #         0.16 * F.mse_loss(reconstructed_feature, original_feature)
    #     )
    ...
    #     # 3. 改进的SSIM损失
    #     ssim_loss = 1 - ssim(reconstructed_feature, original_feature, data_range=1.0, size_average=True)
    ...
    #         self.discriminator, original_feature, reconstructed_feature
    #     )
        
    #     d_loss = (torch.mean(d_fake) - torch.mean(d_real) + 10.0 * gradient_penalty)
    #     g_loss = -torch.mean(self.discriminator(reconstructed_feature))
        
    #     # 6. 组合所有损失
    #     total_loss = {
    #         'recon_loss': self.distill_cfg['lambda_recon'] * (
    #             recon_loss + 
    #             0.8 * ssim_loss + 
    #             0.2 * perceptual_loss
    #         ),
...
class WaveletGenerator11(nn.Module):
    def __init__(self, out_channels=64):
        super().__init__()
...
class WaveletDiscriminator11(nn.Module):
    def __init__(self, in_channels=64):
        super().__init__()
...
#         self.decoder = nn.Sequential(
#             nn.Conv2d(64, 128, 3, padding=1),
#             nn.InstanceNorm2d(128),  # 替换为Instance Normalization
